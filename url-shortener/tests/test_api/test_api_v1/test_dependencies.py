...
class DependsTestCase(TestCase):
    ...
    def test_prefetch_url(self):
        slugs = {su.slug for su in storage.get()}
        for slug in slugs:
            prefetch_url(slug)
            self.assertTrue(slug)

    def test_prefetch_url_film(self):
...
class TestUnsafeMethods(TestCase):
    def test_unsafe_methods_doesnt_contain_safe_methods(self):
        safe_methods = {
            "GET",
            "OPTIONS",
            "HEAD",
    ...
    def test_uppercase_unsafe_metgods(self):
